...
class SaboteurController {
  ...
  */
  getPermission(message){
    if (message.author.id in config.blacklist) { return config.permission.b; }
    if (message.author.id in config.whitelist) { return config.permission.w; }
    if (message.member._roles.find((r) => r in config.admins)){ return config.permission.a; }
  ...
  handleMessage(message) {
    ...
    this.db.register_message(message);
  ...
  handleCommand(command, message, text, permission) {
	...
	this.handleCommand(command.subCommand[parsed.first], message, parsed.rest, command.subCommand[parsed.first], permission);
      }
    }
    // register command in db
    command.action.call(command, message, text, this.db, permission); // some ation can trigger command AND args
  }
